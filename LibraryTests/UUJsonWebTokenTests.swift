...
final class UUSignedJsonWebTokenTests: XCTestCase
{
  ...
  func test_parse_stripsBearerPrefix()
  {
    switch UUSignedJsonWebToken.parse("Bearer \(JwtTestVectors.signedToken)")
    {
      case .success(let token):
        XCTAssertEqual(token.compactSerialization, JwtTestVectors.signedToken)
  ...
  func test_parse_stripsCaseInsensitiveBearerPrefix()
  {
    switch UUSignedJsonWebToken.parse("bearer \(JwtTestVectors.signedToken)")
    {
      case .success(let token):
...
final class UUEncryptedJsonWebTokenTests: XCTestCase
{
  ...
  func test_parse_trimsWhitespaceAndBearerPrefix()
  {
    switch UUEncryptedJsonWebToken.parse("  Bearer \(JwtTestVectors.encryptedToken)  ")
    {
      case .success(let token):
        XCTAssertEqual(token.compactSerialization, JwtTestVectors.encryptedToken)
