...
fileprivate let LOG_TAG = "UUCoreData"
...
open class UUCoreData: NSObject
{
    ...
    private func configure(url: URL, modelDefinitionBundle: Bundle = Bundle.main)
    {
        ...
        if (mom == nil)
        {
            NSLog("WARNING! Unable to create managed object model!")
            return
        }
    ...
    private func configure(url: URL, model: NSManagedObjectModel)
    {
        ...
        {
            NSLog("Error setting up CoreData: %@", String(describing: err))
            ...
            {
                NSLog("Error Clearing CoreData: %@", String(describing: innerError))
            }
        }
        ...
        if (destContext != nil)
        {
            ...
            if (savedContext != nil)
            {
                if (savedContext != destContext && savedContext!.persistentStoreCoordinator == mainThreadContext!.persistentStoreCoordinator)
                {
                    destContext!.perform
                    {
                        NSLog("Merging changes from background context")
    ...
    func uuSubmitChanges() -> Error?
    {
        ...
        performAndWait
        {
            if (hasChanges)
            {
                ...
                {
                    error = err
                    NSLog("Error saving core data: %@", String(describing: err))
                }
            }
        }
    ...
    func uuDeleteAllObjects()
    {
        performAndWait
        {
            ...
            for entity in entityList!
            {
                ...
                {
                    NSLog("Error deleting all objects: %@", String(describing: err))
                }
            }
        }
    }
...
public extension NSError
{
    func uuLogDetailedErrors()
    {
        ...
        if (detailedErrors != nil)
        {
            ...
            {
                NSLog("  DetailedError: %@", de.userInfo)
            }
        }
        else
        {
...
public extension NSManagedObject
{
                ...
                if #available(iOS 9.0, *)
                {
                    ...
                    do
                    {
                        if let result = try context.execute(request) as? NSBatchDeleteResult
                        {
                            ...
                            {
                                NSLog("Deleted Object IDS: \(deletedObjectIds)")
                            }
                        }
                    }
                    catch let err
        ...
        logMessage: String = "")
    {
        ...
        for o in list
        {
            ...
            {
                NSLog("\(uuEntityName)-\(i): \(odbg.debugDescription)")
            }
            else
            {
                NSLog("\(uuEntityName)-\(i): \(o)")
            }
    ...
    func uuFill(from dictionary: [AnyHashable:Any], context: Any?)
    {
        ...
        ctx.performAndWait
        {
            ...
            {
                //NSLog("Attribute: \(attr.key) ==> \(attr.value)")
                ...
                switch (attr.value.attributeType)
                {
                    case .stringAttributeType:
                        ...
                        //NSLog("Found String for \(attr.key): \(strValue ?? "null")")
                    ...
                    case .integer16AttributeType,
                        ...
                        //NSLog("Found Number for \(attr.key): \(String(describing: numValue))")
                    ...
                    case .dateAttributeType:
                        ...
                        //NSLog("Found Date for \(attr.key): \(strValue ?? "null")")
                    
                    default:
                        NSLog("Skipping \(attr.key)")
                }
            ...
            {
                //NSLog("Relationship: \(rel.key) ==> \(rel.value)")
...
public extension UUCoreData
{
    ...
    static func destroyStore(at url: URL)
    {
        ...
        {
            NSLog("Error deleting store file: \(err)")
        }
...
public let UUCoreDataErrorDomain = "UUCoreDataErrorDomain"

public enum UUCoreDataErrorCode: Int
{
    ...
    case deleteStoreFailed = 6
    case coreDataError = 7
}

public extension UUCoreDataErrorCode
{
    var errorDescription: String
    {
        switch self
        {
            ...
            case .persistentContainerNotOpen:
                return "Persistent container not open."
                
            case .loadPersistentStoresFailed:
                return "Load persistent stores failed."
            
            case .deleteStoreFailed:
